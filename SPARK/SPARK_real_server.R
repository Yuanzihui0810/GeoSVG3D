...
spark.test.new <- function(object, 
  ...
  if(is.null(kernel_mat)){ #calculate kernel matrix using location information
    ...
    for(ikernel in c(1:5) ){
      # Gaussian kernel
      cat(paste0("## testing Gaussian kernel: ",ikernel,"...returned", nrow(object@res_stest),
                 "genes\n"))
      ...
      # Periodic kernel
      cat(paste0("## testing Periodic kernel: ",ikernel,"...returned", nrow(object@res_stest),
                 "genes\n"))
    ...
    for(ikernel in 1:length(kernel_mat) ){
      # pre-defined kernels
      cat(paste0("## testing pre-defined kernel: ",ikernel, ".returned", nrow(object@res_stest),
                 "genes\n"))
...
data_dir <- "/public3/home/scg5453/zhyuan/Deep_STARMAP"
...
out_dir <- "/public3/home/scg5453/zhyuan/GeoSVG_SPARK_output"
...
cat("nrow(counts):", nrow(spark@counts), "\n")
cat("length(res_vc):", length(spark@res_vc), "\n")
cat("identical genes:",
    identical(rownames(spark@counts), names(spark@res_vc)),
    "\n")
