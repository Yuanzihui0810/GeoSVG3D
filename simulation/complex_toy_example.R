...
simulate_svg_continuous_data2 <- function(
    n = 10000,
    G = 1000,
    n_svg = 120,
  ...
  # and then change to irregular 3D tissue coordinates
  # ============================================================
  ...
  # Nonlinear embedding into a curved 3D tissue
  theta <- 1.15*pi * (v - 0.5)  #1.5*pi * (v - 0.5)
  r <- 1 + 0.6 * u

  x_obs <- r * cos(theta)
  y_obs <- r * sin(theta)
  z_obs <- w + 0.25 * sin(2 * pi * v)
...
seed = 1
alpha = 0.01
sim <- simulate_svg_continuous_data2(
  n = 10000,
  G = 1000,
  n_svg = 120,
  sigma_eps = 0.01,  # 0.01
...
write.csv(sim$Y, file = file.path(data_dir, "log_counts2.csv"), row.names = TRUE)
write.csv(sim$S, file = file.path(data_dir, "coords2.csv"), row.names = TRUE)
...
saveRDS(
  ...
  file = file.path(rds_dir, paste0("Geosvg3d_simulation2_results_seed", seed, ".rds"))
)
